  ...
  const resetSidebarWidth = (timeout: number) => {
    if (sidebarRef.current && navRef.current) {
      ...
      navRef.current.style.setProperty(
        "left",
        isMobile ? "0" : isTablet ? "240px" : isDesktop ? "240px" : "0",
      );
      navRef.current.style.setProperty(
        "width",
        isMobile
  ...
  return (
    <>
      <aside
        ref={sidebarRef}
        className={cn(
          "group/sidebar relative z-[99999] flex min-h-dvh w-60 flex-col overflow-y-auto rounded-e-3xl bg-primaryContainer/15 text-onSurfaceVariant transition-colors duration-250 sm:max-w-sm md:max-w-md dark:bg-surfaceContainer",
        ...
        >
          ...
          <div className="mt-4 w-full rounded-2xl px-2">
            <Button
              ...
              iconPosition="start"
              size="full"
              className="bg-[#dae2ff] text-[#001847]"
      ...
      <div
        ref={navRef}
        className={cn(
          "absolute left-60 top-0 z-[99999] w-[calc(100%-240px)] bg-surfaceContainer",
          isResetting && "transition-all duration-[450ms] ease-out",
          isMobile && isTablet && "left-0 w-full",
        )}
      >
        <nav className="mx-auto w-full max-w-screen-2xl bg-transparent px-3 py-2">
          {isCollapsed ? (
            <Button
              variant="text"
              size="icon"
